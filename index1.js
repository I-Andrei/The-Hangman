...
function inputNewWord(){
 ...
 function inputLetter(){
  let letter = document.getElementById("newLetter").value
  let found = 0, completed = 1
  for (let i = 1; i < newWord.length - 1; ++i){
    if (newWord.charAt(i) == letter){
      let wordReady2 = newWord.charAt(0)
      for (let s = 1; s < newWord.length - 1; ++s){
        if (s == i)
          wordReady2 += letter;
        else
